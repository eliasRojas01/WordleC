...
int main(){
  char misterious_word[WORD_LENGTH], guess[WORD_LENGTH], try_feedback[WORD_LENGTH-1];   /*en misterious_word se almacenara la palabra a
                                                                                          adivinar ,en guess el intento y try_feedback
  ...
  char tracking[WORD_LENGTH-1] = {'-'};                                                 /*cada numero representara la posicion de cada
  ...
  }while( !(wanna_play == YES || wanna_play == NO) );           /*si wanna_play es igual a N o a S, salgo del loop*/

  if( wanna_play == NO ){                                       /*si el usuario ingreso N, el programa termina*/
    printf("Bye bye\n");
    return EXIT_SUCCESS;
  }
  ...
  for(i = 0; i < total_games; i++){                             //i sera el numero de partida ;
    getWordInLine(EXTERNAL_FILE,rand()%get_file_lines(EXTERNAL_FILE)+1, misterious_word);       /* obtengo una palabra "random" del 
                                                                                                   archivo palabras.txt utilizando 
                                                                                                   get_file_lines, que me devuelve 
                                                                                                   las lineas totales del archivo a
                                                                                                   leer*/
    ...
    for(j = 0 ; j < TRIES_PER_GAME; j++){                                                       //j seran los intentos por partida
      ...
      clean_stdin();                                            /* un buffer es un area temporal de almacenamiento,todos los dispositivos
                                                                   tienen buffers de entrada y salida, cuando entra mas informacion de la
                                                                   necesaria, esta queda guardada en el buffer de entrada, entonces ni bi
                                                                   en se vuelva a pedir input, esta informacion "basura" sera tomada por
                                                                   la proxima funcion. Por esto uso clean_stdin(),encontrada en github
                                                                   por cierto, para que el proximo loop no agarre "basura" del anterior.
                                                                   */
      ...
      scores[i] += set_score(tracking, try_feedback);           /*set_score obtendra la puntuacion de cada jugada y ademas se le 
                                                                  pasara el tracking para checkear si el usuario gano o no con user_won*/


      if(user_won(try_feedback) ){                              /*si el usuario gana seteo el score de esa partida a 10000*/
        if(j==0)
    ...
    if(j == TRIES_PER_GAME){                                    /* si hizo 6 intentos y no gano, pierde*/
    ...
    if(keep_playing == NO){
      i++;                                                      //si el usuario sale antes, agregar +1 al contador de partidas jugadas
      break;
    }
  }
  printf("\n-----ESTADISTICAS-----\n");
...
int set_score(char tracking[],char try_feedback[]){             /*recibira tracking para ver que letras fueron adivinadas ya, si la
                                                                  letra fue adivinada, no contara en el puntaje porque se marco previamen
                                                                  te en tracking. Tambien recibira el intento de la jugada actual.*/
  int try_score = 0, i;
...
void print_feedback(char feedback[],char word[], char guess[]){
  ...
  while(i < WORD_LENGTH-1){                                     //loopeo por toda la string de manera que:
    if( word[i] == guess[i] ){                                  /*si encuentro caracteres iguales en la misma posicion de la palabra a
                                                                  adivinar y el intento, printeo + y completo feedback[i]*/
      feedback[i] = '+';
      printf("+");
    }else if(strchr(word, guess[i]) != NULL){                   /*si encuentro que el caracter i del intento, se encuentra en la palabra
                                                                  a adivinar, printeo * y completo feedback[i]*/
      feedback[i] = '*';
      printf("*");
    }else{                                                      /* si definitivamente el caracter no esta en la palabra a adivinar,
...
int user_won(char try_feedback[]){                              /*si detecta una seguidilla de 5 '+' en el intento actual, el usuario 
...
void print_lower_score(int scores[],int games){
  int i, lowest_num;
  int game = 1;                                                 /* guarda en que juego se obtuvo el menor puntaje, inizializado en 1
                                                                   porque si no se llegara a encontrar algun puntaje mayor que el primero
                                                                   , tendria basura dentro*/
  for(i = 0; i < games; i++){
...
int get_file_lines(char file_name[]){
  ...
  while((file_char = fgetc(fp)) != EOF){                        /*fgetc lee el siguiente caracter de fp y lo devuelve como un unsigned
                                                                  char casteado a int o EOF al final de linea o en un error*/
